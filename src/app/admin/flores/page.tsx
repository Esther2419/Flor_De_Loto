...
// 1. Colores primarios (Botones rápidos - Recuperado de tu código original)
...
// 2. Referencia para detección automática (Recuperado completo)
...
export default function FloresAdminPage() {
  ...
  // Validación de Stock automática (Recuperada)
  useEffect(() => {
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm animate-in fade-in slide-in-from-bottom-2">
            ...
            <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-3xl p-8 bg-gray-50 overflow-hidden relative min-h-[250px]">
                    {uploading ? (
                        <span className="text-xs font-bold text-[#C5A059] animate-pulse uppercase tracking-widest">Analizando imagen...</span>
                    ) : formData.foto ? (
                        <div className="relative w-40 h-40 rounded-full overflow-hidden border-4 border-white shadow-lg">
                            <Image src={formData.foto} alt="Preview" fill className="object-cover" unoptimized />
                            <button type="button" onClick={() => { deleteImageFromStorage(formData.foto); setFormData({...formData, foto: ""})}} className="absolute top-0 right-0 bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600 transition-colors"><Trash2 size={16} /></button>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-4 w-full max-w-xs">
                             <button type="button" onClick={() => cameraInputRef.current?.click()} className="flex items-center justify-center gap-2 bg-[#0A0A0A] text-[#C5A059] py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-all">
                              ...
                              <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center gap-2 bg-white text-gray-600 border border-gray-200 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-gray-50 transition-all">
                                <ImageIcon size={20} /> Elegir Galería
                              </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input required type="number" step="0.5" value={formData.precio} onChange={e => setFormData({...formData, precio: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-lg p-3 text-[#0A0A0A] outline-none" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Cantidad (Stock)</label>
                    <input required type="number" value={formData.cantidad} onChange={e => setFormData({...formData, cantidad: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-lg p-3 text-[#0A0A0A] outline-none" />
                  </div>
                ...
                <button disabled={loading || uploading} type="submit" className="w-full bg-[#0A0A0A] text-[#C5A059] py-4 rounded-xl font-bold tracking-[0.2em] uppercase hover:bg-[#C5A059] hover:text-white transition-all shadow-lg">
            ...
            {flores.map((flor) => (
                <div key={flor.id} onClick={() => setSelectedFlor(flor)} className={`bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group relative cursor-pointer ${!flor.disponible ? 'border-red-100 opacity-80' : 'border-gray-100'}`}>
                    ...
                    <div className="relative h-40 md:h-48 bg-gray-50">
                        ...
                        <div className="absolute bottom-2 right-2 z-10">{flor.disponible ? <span className="bg-[#25D366]/90 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Disponible</span> : <span className="bg-red-500/90 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Agotado</span>}</div>
                    </div>
                    <div className="p-4">
      ...
      {selectedFlor && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedFlor(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <button onClick={() => setSelectedFlor(null)} className="absolute top-4 right-4 z-10 bg-black/20 text-white rounded-full p-1"><X size={20}/></button>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex flex-wrap gap-3 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span className="bg-gray-100 px-2 py-1 rounded flex items-center gap-1">Color: <div className="w-2 h-2 rounded-full" style={{backgroundColor: getColorStyle(selectedFlor.color)}}></div> {selectedFlor.color}</span>
