...
export default async function AdminPedidosPage({ 
  ...
  const [rawPedidos, rawFlores, rawEnvolturas] = await Promise.all([
  ...
  const flores = JSON.parse(JSON.stringify(rawFlores, (key, value) =>
    typeof value === 'bigint' ? value.toString() : value
  ));

  const envolturas = JSON.parse(JSON.stringify(rawEnvolturas, (key, value) =>
    typeof value === 'bigint' ? value.toString() : value
  ));
